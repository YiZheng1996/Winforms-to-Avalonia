...
现场限制：无真实 UOS、PLC、USB-RS485、Modbus TCP 仪表和现场 OPC UA。所有带 `PASS-OFFLINE` 的结果均不能替代现场验收。
...
| M-05 | Modbus RTU/TCP 读取 | 未执行 | `BLOCKED` | USB-RS485/串口、终点、功能区、站号、报文、字节序 |
...
以下不能从本轮自动化结果推导：PLC/Modbus 协议与地址正确性、UOS 兼容、systemd 非 root 生命周期、本机 OPC UA 证书/地址空间、连续 2 小时稳定性、现场拔插、真实登录/权限、电气联锁、写入/回读、数据库恢复和报表渲染。
