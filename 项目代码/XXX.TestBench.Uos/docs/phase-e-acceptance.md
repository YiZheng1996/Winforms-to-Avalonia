...
日期：2026-08-31
验收范围：当前已实现的 Core、Gateway 只读 Runtime、Avalonia 8 个任务页面/51 行 UI 覆盖、行为/Headless 测试和离线发布物
...
- `PASS-UI-COVERAGE`：Legacy 界面有唯一映射且迁移后的纯 UI 行为通过；不代表外部数据库、报表或设备效果已实现。
...
| CORE-02 | 启动失败 | 配置/Gateway 错误进入 Faulted、保留原始启动诊断且不吞错 | VM 行为测试构造 Runtime 不可用场景，结果为 Faulted，反馈保留启动错误 | `PASS-OFFLINE`（选择 Faulted 窗口策略） |
| UI-01 | 产品输入/双向绑定 | 输入框双向绑定，选择动作才提交 Core，长度超限拒绝 | Headless 双向绑定、VM/Core 测试 | `PASS-OFFLINE`（非数据库型号选择） |
| UI-02 | Legacy Designer 全覆盖 | 51 个 Designer 各有唯一 Avalonia 目标、状态、行为/Headless 证据、安全处置和剩余缺口；所有证据 ID 可定位到真实测试/脚本唯一 Marker | `legacy-ui-coverage.csv` + `ui-test-evidence.csv` + `Test-MigrationEvidence.ps1` | `PASS-UI-COVERAGE` |
| UI-03 | 主导航/可达性 | 8 个任务页由同一 ViewModel 状态切换，Ctrl+1～Ctrl+8 与 ListBox 共用导航路径 | VM 验证 8 个唯一键；Headless 真实发送 8 个 Ctrl 快捷键并验证 ListBox 选择 | `PASS-UI-COVERAGE` |
| UI-04 | 试验/参数/报表交互 | 21 个试验类唯一映射；9 个参数模块按 Legacy 专项字段/权限勾选/项点双列表/型号发布实现会话交互；组合筛选和报表页码边界 | Avalonia VM 测试；Headless 产品提交、管理专项控件、报表查询 | `PASS-UI-COVERAGE`（认证、持久化、动态 C# 与外部发布阻断） |
| UI-05 | 写入型界面安全锁定 | 工艺 VX/DO/AO、复位、Zero/Gain 应用、Legacy 6 个 AO 槽位、绝缘仪器连接/发送均不可执行 | VM `CanExecute=false`/6 槽位全禁用；Headless 检查危险按钮禁用 | `PASS-OFFLINE`（写未实现） |
| UI-06 | 布局/可访问性/本地化边界 | 最小/宽窗口均有滚动容器；关键入口有 AutomationProperties；导航文本有 Presentation 资源边界 | Headless narrow/wide 与可访问名称；`PresentationTextCatalog` | `PASS-UI-COVERAGE`；UOS 手工 QA 仍待现场 |
| UI-07 | Legacy 日志筛选 | 默认当天；七个 Legacy 等级语义；日期查询、等级即时筛选、Information/Warning 别名、倒序和 500 条边界；关键字明确为增强 | Avalonia VM 日期边界/别名/容量测试；Headless 日期、等级、查询和列表控件 | `PASS-UI-COVERAGE`（历史 SQLite/真实身份来源阻断） |
...
| PKG-01 | Windows 发布启动 | `win-x64` 自包含包启动，Host 用包内 config `--once` 返回 0 | 本轮验证基线 `92df619e62b3ee76e59630ba7b734ddd36592cf1` 重新生成的 Windows 包；Host 返回离线五点快照，HMI 存活 3 秒 | `PASS-OFFLINE`；仅 Windows 离线进程证据 |
...
| M-02 | 发布版 Windows 启动 | 本轮验证基线生成的 `XXX.TestBench.Avalonia.exe` 隐藏启动后存活 3 秒，随后按精确 PID 定向结束 | `PASS-WINDOWS-PROCESS-START` | 正常关闭和长时间运行记录；此烟测不替代 Headless 页面行为或 UOS 验收 |
...
| M-07 | 1920×1080 触控/字体 | Windows Headless 已验证 8 页、44 高度通用交互目标、960×560 最小/1600×900 宽窗口、滚动容器、快捷键和 AutomationProperties；未做真实触控 | `PENDING-FIELD` | 目标屏截图、触控命中、字体/缩放、焦点顺序和本地化记录 |
| M-08 | Legacy 同输入对照 | 只有旧源代码/OPF 证据；未在同一真实输入下并行运行 | `BLOCKED` | 分时对照值、质量、时间戳、地址和报表差异 |

## 3. 明确未验收项

本轮已证明 51 个 Designer 有唯一迁移处置、8 个任务页可达且纯界面行为通过，也证明信捷终点可以在 UnitId=1 候选下响应四个仅读请求；但不能由此推导：参数管理已持久化、报表/打印/上传已可用、完整试验动作已实现，或 PLC 系列与 UnitId 已确认、`AI.MAI00`/`DI.MDI00`/`Modbus.WSD.CH00` 业务映射正确、HD1074 浮点字序正确、UOS 兼容、systemd 非 root 生命周期、本机 OPC UA、连续 2 小时稳定性、现场拔插、真实登录/权限、电气联锁及任何写入/回读已验收。
